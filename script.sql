...
CREATE PROCEDURE P_UPDATE_CHANGES(code VARCHAR2) AS
  BEGIN
    ...
    IF SQL%ROWCOUNT = 0
    THEN
      INSERT INTO CHANGES (CODE, CHANGED) VALUES (code, sys_extract_utc(systimestamp));
    END IF;
  END;
...
ALTER TABLE APPLICATION
  ADD CONSTRAINT APPLICATION_PNS_CH CHECK (PNS IN ('FCM', 'APNs', 'WNS', 'MPNS'));

CREATE UNIQUE INDEX APPLICATION_OS_PN_I
  ON APPLICATION (PNS, PACKAGE_NAME);
...
CREATE TABLE APPLICATION_FCM (
  ID         NUMBER(20)   NOT NULL,
  SERVER_KEY VARCHAR2(64) NOT NULL
);

ALTER TABLE APPLICATION_FCM
  ADD CONSTRAINT APPLICATION_FCM_FK
